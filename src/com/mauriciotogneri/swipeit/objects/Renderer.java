...
public class Renderer implements android.opengl.GLSurfaceView.Renderer
{
	...
	public Renderer(Context context, final Game game, GLSurfaceView surfaceView)
	{
		...
		surfaceView.setOnTouchListener(new GestureManager()
		{
			...
			{
				game.onTap(getScreenX(x), getScreenY(y));
			}

			@Override
			public void onSwipeUp(float x, float y)
			{
				game.onSwipeUp(getScreenX(x), getScreenY(y));
			}

			@Override
			public void onSwipeDown(float x, float y)
			{
				game.onSwipeDown(getScreenX(x), getScreenY(y));
			}
			
			@Override
			public void onSwipeLeft(float x, float y)
			{
				game.onSwipeLeft(getScreenX(x), getScreenY(y));
			}
			
			@Override
			public void onSwipeRight(float x, float y)
			{
				game.onSwipeRight(getScreenX(x), getScreenY(y));
			}
		});
	}
	...
	public void onDrawFrame(GL10 unused)
	{
		...
		this.game.draw(this.positionLocation, this.colorLocation);
	}
}